...
def _cf_get(url, **kwargs):
    """Fetch an APKMirror page, bootstrapping a normal browser session once."""
    ...
    if response.status_code == 403:
        ...
        if response.headers.get("cf-mitigated") == "challenge" or "cloudflare" in body:
            try:
                from src.browser_session import get_cookies

                cookies = get_cookies(url, ".apkmirror.com")
                for name, value in cookies.items():
                    session.cookies.set(name, value, domain=".apkmirror.com")
                if cookies:
                    response = session.get(url, **kwargs)
            except Exception as exc:
                logging.debug("APKMirror browser bootstrap failed: %s", exc)
            if response.status_code == 403:
                _blocked_by_cloudflare = True
                logging.warning("APKMirror requires interactive verification on this runner")
